...
credible_interval1
```
The interpretation of prior in is $\theta$ Beta(10,10) is that before data is observed, the unknown parameter is modeled as a
random variable $\theta$ follows a distribution Beta(10,10), we assuming we know more in this question, since more people will be observed in this case (20-2=18 more women), so we will know more information.
...
p <- x/n
data <- data.frame(theta = theta_values,dbinom(118, size = n, prob = p))
...
```
From the plot, we can find that the peak of likelihood around 0.9 and shows a slightly left skewed trend. For Beta(1,1) prior, which implies that before observing data,we consider all values of $\theta$ equally, thus it is a noninformative prior, however, for Beta(10,10) prior, we can find the peak of it is at $\theta$=0.5, which shows that women indicating they are happy are most likely around 50%. Moreover, we noticed that after observing data, the posterior is shifting to around 0.9 which is closer to the peak of data.
